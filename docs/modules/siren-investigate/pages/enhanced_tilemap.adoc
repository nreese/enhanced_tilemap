...
=== Layer Control
The Layer Control is located in the top right of any Enhanced Coordinate Map. An example is in the image below showing a case where some Stored Layers are loaded (See the <<Loading Stored Layers from Elasticsearch Onto Enhanced Coordinate Map, Loading stored layers from Elasticsearch>> for more details). 
...
By default only the `sirenadmin` user has the permissions to view the map layer indices, to make them available for other users you will need to give their roles read permissions.
You can do that by adding `?map__*` to allowed indices on the Access Control page.

image::ac_map_layers_read_permissions.png[image]

NOTE: This will match all map indices. You can also add each index individualy.

==== Document Level Security (DLS)

Document level security can be done on the map indices, this will only allow documents matching the DLS query to be returned.

WARNING: To maintain good performance you should run the dls map queries on the map indices, separetly of other indices, link:#full_dls_example[example]. More on that link:https://docs.search-guard.com/latest/document-level-security#performance-considerations[ here].

*By spatial path*

This dls query will only allow documents with _"World Lakes"_ in their `spatial_path`.
[#full_dls_example]
....
index_permissions:
  - index_patterns:
...
This dls query will only allow documents within the specified co-ordinates.
....
- index_patterns:
    - '?map__*'
  dls: >-
...
This dls query will only allow documents that match a specific property.
....
- index_patterns:
    - '?map__*'
