...
This is where configurations for Stored Layers are defined. See <<Loading Stored Layers into Elasticsearch>> for more details on ingesting Stored Layer Data. The following parameters can be configured: 

* *Spatial path* - A path that the configuration usesshould correspond to a layer

NOTE: One configuration is allowed to omit a spatial_path attribute. This is the default configuration and values will be taken from it if they are not found within the <<Cascading Configuration Process>>

* *Icon* - Favicon to display for points on map and layer control
* *Color* - Color to display on map and layer control
* *Size* - Size to display points on map
* *Popup Fields* - The content of the tooltip that appears when mousing over features
...
NOTE: See here for https://leafletjs.com/examples/zoom-levels/[more detail on Zoom levels]

===== Cascading Configuration Process
Assigning configurations for each layer is done via a cascading process, similar to CSS, whereby the most specific spatial_path to the given layer is checked for the presence of the configuration parameters first. Then the spatial_path chain is followed and defaults will be assigned if a parameter is missing a configuration.  

For more information, see the <<More Detailed Stored Layer Configurations Example>>

===== Parameter and Field Configurations
It is possible to configure some parameters either using the parameter or field approaches. The parameter approach is where the exact parameter to use on the map is outlined. It is also possible to configure a field that exists within the documents of each layer. Examples of both approaches are outlined in the table below. Parameters are configured by a string, while a single element array is used for the field type approach. 

.Parameter configuration examples
|===
|Parameter |Parameter examples |Field example 
...
NOTE: When a new Enhanced Coordinate Map visualzation is created, the Stored Layer Configuration will be popuplated with a default object: image:default_default_stored_layers_config.png[Default default stored layers configuration]

===== Feature level configurations
If either icon, size, color or popupField are specified in a document's properties object, this field will take precedence. This may be useful for assigning different icons witin the same layer
...
A number of Stored Layer parameters can be configured from the UI <<Stored Layer Configurations, when in Visualization edit mode>>
...
=== More Detailed Stored Layer Configurations Example

The icon, color, size or popupFields can be specified in the properties object of an ES document and this is the first precedence. For example, the church icon below will be a `red heart` on the map
...
.Example of document with some configuration fields (example 1)
...
.Example of the Stored Layer Configuration (example 2)

* Values for icon, size, popupFields and color properties on the map are taken from
** Feature itself 
** Then from the the most specific spatial_path in the configuration
** Then from the next most relevant spatial_path in the configuration
** Until we reach the root of the configuration where a default will be assigned
* Values for maxZoom, minZoom properties on the map are taken from
** Begins at the most specific spatial_path in the configuration
** Then from the next spatial_path in the configuration
** Until we reach the root of the configuration where a default will be assigned
* Values for Icon and color for layer on the <<Layer Control>>
** Same as for maxZoom, minZoom properties
...
* *color*: `green` - field type, taken from the configuration and retrived from a document with the “pois/states/churches” spatial_path
