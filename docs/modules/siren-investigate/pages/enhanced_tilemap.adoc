...
Point of Interest layers::
  ...
  * Geo_point type POI layers are drawn using the <<Marker Clustering>> process.
...
[NOTE]
==== 
Point layers from WFS sources will render single document features rather than the <<Marker Clustering>> support for other point layer types
====
...
==== Marker Clustering
Point layers from <<Point of Interest layers, Point of Interest>> and <<Loading Stored Layers into Elasticsearch, Stored Layer>> sources use Marker Clustering. This approach means that all documents on the current map canvas are represented at once. 

===== How does it work?
Two queries are used each time this layer type is re-rendered. Initially, a geohash aggregation query is fired. Based on this, geohashes that have the least number of documents present are selected up to the point that the configured limit would be exceeded. These selected geohashes are removed and Geo-Filters are created for these instead. A document query is then fired to retrive individual documents within these areas. The configured aggregation <<The Options Tab, Map Collar Scale>> is used for both queries.

===== How does it look?
The result is demonstrated below, the cluster can be distinguished from the number to the right (391). When a cluster is hovered over, the corresponding geohash appears. When a marker corresponding to one document is hovered over, a popup, if configured (see <<Configuring Stored Layers, Stored Layers>> and <<Point of Interest layers, POI>> for respective configurations) appears.

image:marker_clustering.png[Marker Clustering Example]
