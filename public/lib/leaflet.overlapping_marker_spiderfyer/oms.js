  ...
  (function () {
    ...
    return this.OverlappingMarkerSpiderfier = (function () {
      ...
      p.clearMarkers = function () {
        let i, j, len, marker, markerListener, ref;
        this.unspiderfy();
        ref = this.markers;
      ...
      p.trigger = function () {
        let args, event, ref, ref1, arr;
        event = arguments[0], args = 2 <= arguments.length ? slice.call(arguments, 1) : [];
        ref1 = (ref = this.listeners[event]) != null ? ref : [];
        if (ref1.length >= 1) {
          ref1[0].apply(null, args);
        }
        (arr = []).length = ref1.length;
        return arr.fill(undefined);
      };

      p.generatePtsCircle = function (count, centerPt) {
        let angle, angleStep, circumference, i, j, legLength, ref, results;
      ...
      p.generatePtsSpiral = function (count, centerPt) {
        let angle, i, j, legLength, pt, ref, results;
      ...
      p.spiderListener = function (marker) {
        let j, len, m, mPt, markerPt, markerSpiderfied, nearbyMarkerData, nonNearbyMarkers, pxSq, ref;
      ...
      p.spiderfy = function (markerData, nonNearbyMarkers) {
        let bodyPt, footLl, footPt, footPts, leg, marker, md, mhl, nearestMarkerDatum, numFeet, spiderfiedMarkers;
        ...
        footPts = numFeet >= this.circleSpiralSwitchover ?
          this.generatePtsSpiral(numFeet, bodyPt).reverse() : this.generatePtsCircle(numFeet, bodyPt);
      ...
      p.unspiderfy = function (markerNotToMove) {
        let j, len, marker, mhl, nonNearbyMarkers, ref, unspiderfiedMarkers;
      ...
      p.minExtract = function (set, func) {
        let bestIndex, bestVal, index, item, j, len, val;
