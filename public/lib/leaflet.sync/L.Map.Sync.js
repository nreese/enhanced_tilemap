...
(function () {
  ...
  L.Sync.offsetHelper = function (ratioRef, ratioTarget) {
    const or = L.Util.isArray(ratioRef) ? ratioRef : [0.5, 0.5];
    const ot = L.Util.isArray(ratioTarget) ? ratioTarget : [0.5, 0.5];
    return function (center, zoom, refMap, targetMap) {
      const rs = refMap.getSize();
      const ts = targetMap.getSize();
      const pt = refMap.project(center, zoom)
  ...
  L.Map.include({
    ...
    isSynced: function (otherMap) {
      let has = (this.hasOwnProperty('_syncMaps') && Object.keys(this._syncMaps).length > 0);
      if (has && otherMap) {
      ...
      L.extend(originalMap, {
        setView: function (center, zoom, options, sync) {
          ...
          function sandwich(obj, fn) {
            let viewpreresets = [];
            const doit = options && options.disableViewprereset && obj && obj._events;
            if (doit) {
          ...
          const ret = sandwich(this, function (obj) {
        ...
        originalMap._syncMaps.forEach(function (toSync) {
          ...
          toSync.eachLayer(function (layer) {
            if (layer._google !== undefined) {
              const offsetFn = originalMap._syncOffsetFns[L.Util.stamp(toSync)];
              const center = offsetFn(originalMap.getCenter(), originalMap.getZoom(), originalMap, toSync);
