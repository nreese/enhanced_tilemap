...
export default function EnhancedTileMapVisType(Private, getAppState, courier, config) {
  const VislibVisType = Private(VislibVisTypeVislibVisTypeProvider);
  var Schemas = Private(require('ui/Vis/Schemas'));
  const geoJsonConverter = Private(AggResponseGeoJsonGeoJsonProvider);

  return new VislibVisType({
    name: 'enhanced_tile_map',
    title: 'Tile map',
    icon: 'fa-map-marker',
    description: 'Custom tile map',
    params: {
      defaults: {
      ...
      editor: require('plugins/enhanced-tilemap/tile_map_options.html')
    },
    listeners: {
